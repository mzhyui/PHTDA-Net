...
def getTopoLoss(likelihood):
    ...
    for y in range(0, likelihood.shape[0], topo_size):
        for x in range(0, likelihood.shape[1], topo_size):
            ...
            if (torch.min(patch) == 0 or torch.max(patch) == -1):
                continue

            pd_lh, bcp_lh, dcp_lh = compute_persistence_2DImg_1DHom_lh(
                patch, 2, 0)
            if (len(pd_lh) == 0):
                continue
            ...
            if (len(idx_holes_to_fix) > 0 or len(idx_holes_to_remove) > 0):
                ...
                for hole_indx in idx_holes_to_fix:
                    ...
                    if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[0] and int(
                        ...
                        topo_cp_ref_map[y + int(bcp_lh[hole_indx][0]),
                                        x + int(bcp_lh[hole_indx][1])] = 0
                    ...
                    if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                        ...
                        topo_cp_ref_map[y + int(dcp_lh[hole_indx][0]),
                                        x + int(dcp_lh[hole_indx][1])] = 1
                    ...
                    if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[
                        ...
                        if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                            ...
                            topo_cp_ref_map[y + int(bcp_lh[hole_indx][0]), x + int(bcp_lh[hole_indx][1])] = \
                                likelihood[int(dcp_lh[hole_indx][0]), int(
                                    dcp_lh[hole_indx][1])]
                        else:
                            topo_cp_ref_map[y + int(bcp_lh[hole_indx][0]),
                                            x + int(bcp_lh[hole_indx][1])] = 1
    ...
    topo_cp_weight_map_tensor = torch.tensor(
        topo_cp_weight_map, dtype=torch.float).cuda()
    topo_cp_ref_map_tensor = torch.tensor(
        topo_cp_ref_map, dtype=torch.float).cuda()
    loss_topo = (((likelihood * topo_cp_weight_map_tensor) -
                 topo_cp_ref_map_tensor) ** 2).sum()
